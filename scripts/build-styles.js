...
async function build(cb) {
  ...
  let lessContent = fs.readFileSync(
  ...
  // Write file
  fse.writeFileSync(
    `./${env === "development" ? "build" : "dist"}/css/swiper.css`,
    `${banner}\n${cssContent}`
  );
  ...
  glob("**/*.*", { cwd: path.resolve(__dirname, "../src") }, (err, files) => {
    files.forEach((file, index) => {
      ...
      let fileContent = fse.readFileSync(
      ...
      fse.writeFileSync(path.resolve(__dirname, "../dist", file), fileContent);
