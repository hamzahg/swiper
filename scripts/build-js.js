  ...
  rollup
    ...
    .then(bundle =>
      bundle.write({
        ...
        sourcemap: true,
        sourcemapFile: `./${
          env === "development" ? "build" : "dist"
        }/js/swiper.esm.bundle.js.map`,
        banner,
        file: `./${
          env === "development" ? "build" : "dist"
        }/js/swiper.esm.bundle.js`
      })
    )
    .then(() => {
      if (cb) cb();
  ...
  rollup
    ...
    .then(bundle =>
      bundle.write({
        ...
        sourcemapFile: `./${
          env === "development" ? "build" : "dist"
        }/js/swiper.esm.browser.bundle.js.map`,
        banner,
        file: `./${
          env === "development" ? "build" : "dist"
        }/js/swiper.esm.browser.bundle.js`
      })
    )
    .then(bundle => {
      if (env === "development") {
      ...
      fs.writeFileSync(
        "./dist/js/swiper.esm.browser.bundle.min.js",
        minified.code
      );
      fs.writeFileSync(
        "./dist/js/swiper.esm.browser.bundle.min.js.map",
        minified.map
      );
      if (cb) cb();
    })
      ...
      plugins: [
        replace({
          ...
          "//IMPORT_COMPONENTS": components
            ...
            .join("\n"),
          "//INSTALL_COMPONENTS": "",
          "//EXPORT": `export { Swiper, ${components
            .map(component => component.capitalized)
            .join(", ")} }`
        }),
        resolve({ mainFields: ["module", "main", "jsnext"] })
